...
class Total_stacked_Amount(CustomMetric):
    def _init_(self):
        super()._init_(blockchain='Avalanche', chain='p', name='total_stacked_amount', transaction_type='emitted_utxo',
                       category='Economic Indicators', description='Description', display_name='Total Stacked Amount')
...
class TotalBurnedAmount(CustomMetric):
    def _init_(self):
        super()._init_(blockchain='Avalanche', chain='p', name='total_burned_amount', transaction_type='consumed_utxo',
                       category='Economic Indicators', description='Description', display_name='Total Burned Amount')
...
class stackingDynamicIndex(CustomMetric):
    def _init_(self):
        super()._init_(blockchain='Avalanche', chain='p', name='staking_dynamics_index',
                       transaction_type='emitted_utxo', category='Economic Indicators', description='Description', display_name='Staking Dynamics Index')
...
class StakingEngagementIndex(CustomMetric):
    def _init_(self):
        super()._init_(blockchain='Avalanche', chain='p', name='staking_engagement_index',
                       transaction_type='emitted_utxo', category='Economic Indicators', description='Description',  display_name='Staking Engagement Index')
...
class InterchainTransactionalCoherance(CustomMetric):
    def _init_(self):
        super()._init_(blockchain='Avalanche', chain='c', name='interchain_transactional_coherence',
                       transaction_type='transaction', category='Economic Indicators', description='Description', display_name='Interchain Transactional Coherence')

    def calculate(self, data: pd.DataFrame) -> float:
        # Assuming 'amountCreated', 'sourceChain', and 'destinationChain' are columns in the DataFrame
        total_cross_chain_value = data.loc[
            (data['sourceChain'].notna()) & (data['destinationChain'].notna()), 'amountCreated'].sum()
...
class InterchainLiquidityRatio(CustomMetric):
    def _init_(self):
        super()._init_(blockchain='Avalanche', chain='c', name='interchain_liquidity_ratio',
                       transaction_type='transaction', category='Economic Indicators', description='Description', display_name='Interchain Liquidity Ratio')

    def calculate(self, data: pd.DataFrame) -> float:
        # Assuming 'amountUnlocked', 'amountCreated', 'sourceChain', and 'destinationChain' are columns in the DataFrame
        total_interchain_value = data.loc[
            (data['sourceChain'].notna()) & (data['destinationChain'].notna()), ['amountUnlocked',
                                                                                 'amountCreated']].replace('',
                                                                                                           '0').astype(
            float).sum().sum()
...
class NetworkEconomyEfficiency(CustomMetric):
    def _init_(self):
        super()._init_(blockchain='Avalanche', chain='x', name='network_economy_efficiency',
                       transaction_type='transaction', category='Economic Indicators', description='Description', display_name='Network Economy Efficiency')
