...
def avg_consumed_utxo_amount(blockchain, subchain, date):
    ...
    query = f"SELECT AVG(CAST(amount AS NUMERIC)) FROM {subchain}_consumed_utxos WHERE date = '{date}' AND consumed = TRUE"
    results = execute_query(query)
...
def median_consumed_utxo_amount(blockchain, subchain, date):
    ...
    SELECT PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY CAST(amount AS NUMERIC)) 
    FROM {subchain}_consumed_utxos 
    WHERE date = '{date}' AND consumed = TRUE
    """
    results = execute_query(query)
...
def large_trx(blockchain, subchain, date, threshold):
    """
...
def whale_address_activity(blockchain, subchain, date, threshold):
    """
