...
class WorkflowManager:
    ...
    def run_avalanche_data_workflow(self):
        ...
        try:
            ...
            compute_average_transactions_per_block(avalanche_X_data, date, "X_CHAIN", os.environ.get("DATABASE_CONNECTION"))
            self.logger.info("Computed average transactions per block !")
            compute_total_staked_amount(avalanche_P_data, date, "X_CHAIN", os.environ.get("DATABASE_CONNECTION"))
            self.logger.info("Computed total staked amount !")
            compute_total_burned_amount(avalanche_P_data, date, "X_CHAIN", os.environ.get("DATABASE_CONNECTION"))
            self.logger.info("Computed total staked amount !")
            compute_average_transaction_value(avalanche_C_data, date, "X_CHAIN", os.environ.get("DATABASE_CONNECTION"))
            self.logger.info("Computed average transaction value !")
            compute_large_transaction_monitoring(avalanche_C_data, date, "X_CHAIN", os.environ.get("DATABASE_CONNECTION"))
            self.logger.info("Computed large transaction monitoring !")
            compute_cross_chain_whale_activity(avalanche_C_data, date, "X_CHAIN", os.environ.get("DATABASE_CONNECTION"))
